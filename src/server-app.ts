...
import { LANDING_SITES, SPECIES_CATALOG, CreateBatchSchema, AppendEventSchema } from './types/aqua-seal';
import { storageAdapter } from './lib/storage-adapter';
import { handleUSSDRequest } from './lib/africas-talking-ussd';

/**
 * Builds the Aqua-Seal API as a standalone Express app, with no listener
 * attached and no static-file serving. This is deliberately separated from
 * server.ts (used for local/Codespaces dev with Vite middleware + static
 * `dist/` serving) so the exact same route logic can also be wrapped as a
